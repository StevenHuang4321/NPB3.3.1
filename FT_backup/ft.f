...
c      common /procgrid/ np1, np2, layout_type, np
...
c      common /dbg/ debug, debugsynch, timers_enabled
...
c---------------------------------------------------------------------
     ...
     >                 u2(ntdivnp)
      ...
      pointer(ptr_ntdivnp, ntdivnp)
c      pointer(prt_u0, u0)
c      pointer(ptr_u1, u1)
c      pointer(ptr_u2, u2)
c      pointer(ptr_twiddle, twiddle)
...
c      common /bigarrays/ u0, pad1, u1, pad2, u2, pad3
      pointer(ptr_pad1, pad1)
      pointer(ptr_pad2, pad2)
      pointer(ptr_pad3, pad3)

      integer iter
      pointer(prt_iter, iter)
...
C      double complex test
C      pointer(ptr_test, test)
...
c      pointer(ptr_twiddle, twiddle)
      print *, "ntdivnp:" , ntdivnp
      print *, "11111111loc(twiddle)", loc(twiddle)
C      call unimem_malloc_ft(loc(u0), sizeof(u0))
c      CALL unimem_malloc_ft(loc(u0))
      print *, "33333333loc(twiddle)", loc(twiddle)
...
c---------------------------------------------------------------------
      ...
      call setup(dims, xstart, ystart, zstart,
     >                xend, yend, zend, commslice1, commslice2,
     >                debug, fftblock, fftblockpad,
     >                fftblock_default, fftblockpad_default,
     >                layout_type, layout_0D, layout_1D, layout_2D,
     >                me, me1, me2, niter, np1, np2, np,
     >                timers_enabled)

      call compute_indexmap(twiddle,dims(1,3), dims(2,3), dims(3,3),
     >                      dims, seed, a, pi, alpha,
     >                            layout_type,
     >                            layout_0D, layout_1D, layout_2D,
     ...
     >                                       xstart, ystart, zstart,
     >                                       xend, yend, zend)
      call fft_init (dims(1,1), u, seed, a, pi, alpha, ilog2)
      call fft(1, u1, u0, layout_type,layout_0D, layout_1D, layout_2D,
     >         dims, fftblock_default, fftblockpad_default,
     >               fftblock, fftblockpad, timers_enabled,
     >               transblock, transblockpad,
     >        T_total, T_setup, T_fft, T_evolve, T_checksum,
     >        T_fftlow, T_fftcopy, T_transpose,
     >        T_transxzloc, T_transxzglo, T_transxzfin,
     >        T_transxyloc, T_transxyglo, T_transxyfin,
     >        T_synch, T_init, T_max, ilog2, u, np, np1, np2, 
     >        commslice1, commslice2)
...
c------------------------main loop begin-------------------------------
c      do iter = 1, niter
c         if (timers_enabled) call timer_start(T_evolve)
c         call evolve(u0, u1, twiddle, dims(1,1), dims(2,1), dims(3,1))
c         if (timers_enabled) call timer_stop(T_evolve)
c         if (timers_enabled) call timer_start(T_fft)
c         call fft(-1, u1, u2, 
c     >            layout_type,layout_0D, layout_1D, layout_2D,
c     >            dims, fftblock_default, fftblockpad_default,
c     >               fftblock, fftblockpad, timers_enabled,
c     >               transblock, transblockpad,
c     >        T_total, T_setup, T_fft, T_evolve, T_checksum,
c     >        T_fftlow, T_fftcopy, T_transpose,
c     >        T_transxzloc, T_transxzglo, T_transxzfin,
c     >        T_transxyloc, T_transxyglo, T_transxyfin,
c     >        T_synch, T_init, T_max, ilog2, u, np, np1, np2,
c     >        commslice1, commslice2)
c         if (timers_enabled) call timer_stop(T_fft)
!         if (timers_enabled) call synchup()
c         if (timers_enabled) call timer_start(T_checksum)
c         call checksum(iter, u2, dims(1,1), dims(2,1), dims(3,1),
c     >                    sums, me,
c     >                    timers_enabled, T_synch,
c     >                    xstart, ystart, zstart,
c     >                    xend, yend, zend)
c         if (timers_enabled) call timer_stop(T_checksum)
c      end do
c------------------------main loop end---------------------------------
c      call verify(nx, ny, nz, niter, verified, class, sums, np, me)
c      call timer_stop(t_total)
c      if (np .ne. np_min) verified = .false.
c      total_time = timer_read(t_total)

c      if( total_time .ne. 0. ) then
c         mflops = 1.0d-6*ntotal_f *
c     >             (14.8157+7.19641*log(ntotal_f)
c     >          +  (5.23518+7.21113*log(ntotal_f))*niter)
c     >                 /total_time
c      else
c         mflops = 0.0
c      endif
c      if (me .eq. 0) then
c         call print_results('FT', class, nx, ny, nz, niter, np_min, np,
c     >     total_time, mflops, '          floating point', verified, 
c     >     npbversion, compiletime, cs1, cs2, cs3, cs4, cs5, cs6, cs7)
c      endif
c      if (timers_enabled) call print_timers(T_total, 
c     >        T_setup, T_fft, T_evolve,
c     >        T_checksum,T_fftlow, T_fftcopy, T_transpose,
c     >        T_transxzloc, T_transxzglo, T_transxzfin,
c     >        T_transxyloc, T_transxyglo, T_transxyfin,
c     >        T_synch, T_init, T_max, timer_read,
c     >        me, me1, me2, np1, np2, np)
...
c---------------------------------------------------------------------
      ...
      subroutine setup(dims, xstart, ystart, zstart, 
     >                xend, yend, zend, commslice1, commslice2,
     >                debug, fftblock, fftblockpad,
     >                fftblock_default, fftblockpad_default,
     >                layout_type, layout_0D, layout_1D, layout_2D,
...
c---------------------------------------------------------------------
      ...
      subroutine compute_indexmap(twiddle, d1, d2, d3, dims,
     >                            seed, a, pi, alpha,
     >                            layout_type,
     >                            layout_0D, layout_1D, layout_2D,
     >                            xstart, ystart, zstart,
...
c---------------------------------------------------------------------
      ...
      subroutine print_timers(T_total, T_setup, T_fft, T_evolve, 
     >        T_checksum,T_fftlow, T_fftcopy, T_transpose, 
     >        T_transxzloc, T_transxzglo, T_transxzfin, 
     >        T_transxyloc, T_transxyglo, T_transxyfin, 
     >        T_synch, T_init, T_max, timer_read,
     >        me, me1, me2, np1, np2, np)
...
c---------------------------------------------------------------------
     ...
     >                  fftblock_default, fftblockpad_default,
     >                  transblock, transblockpad,
     >                  fftblock, fftblockpad, T_total, T_setup, T_fft,
     >                  T_evolve, T_checksum,
     >        T_fftlow, T_fftcopy, T_transpose,
     >        T_transxzloc, T_transxzglo, T_transxzfin,
     >        T_transxyloc, T_transxyglo, T_transxyfin,
     >        T_synch, T_init, T_max, timers_enabled,
     >        ilog2, u)
     ...
     >        T_transxzloc, T_transxzglo, T_transxzfin,
     >        T_transxyloc, T_transxyglo, T_transxyfin,
     >        T_synch, T_init, T_max, ilog2, u)
            call cffts3(1, dims(1,3), dims(2,3), dims(3,3), 
     >                  x1, x2, scratch, 
     >                  fftblock_default, fftblockpad_default,
     >                  transblock, transblockpad,
     >                  fftblock, fftblockpad, ilog2,
     >                  T_total, T_setup, T_fft,
     >                  T_evolve, T_checksum,
     >        T_fftlow, T_fftcopy, T_transpose,
     >        T_transxzloc, T_transxzglo, T_transxzfin,
     >        T_transxyloc, T_transxyglo, T_transxyfin,
     >        T_synch, T_init, T_max, timers_enabled, u)
         else if (layout_type .eq. layout_1d) then
            call cffts1(1, dims(1,1), dims(2,1), dims(3,1), 
     >                  x1, x1, scratch,
     >                  fftblock_default, fftblockpad_default,
     ...
     >        T_synch, T_init, T_max, ilog2, u)
            ...
            call transpose_xy_z(2, 3, x1, x2, np, dims,
     >                        T_transxzfin, np2, timers_enabled,
     >                        T_transxzglo, commslice1,
     >                       transblockpad, transblock, T_transxzloc)
     ...
     >        ilog2, u)
            if (timers_enabled) call timer_start(T_transpose)
            call transpose_x_y(1, 2, x1, x2, dims ,
     >                         T_transxyloc, T_transxyglo,T_transxyfin,
     >                         np1, commslice2)
            if (timers_enabled) call timer_stop(T_transpose)
            call cffts1(1, dims(1,2), dims(2,2), dims(3,2), 
     ...
     >                           commslice1, T_transxzglo, T_transxzfin,
     >                           np2, np1,np, T_transxzloc)
            if (timers_enabled) call timer_stop(T_transpose)
            call cffts1(1, dims(1,3), dims(2,3), dims(3,3), 
     >                  x1, x2, scratch,
     >                  fftblock_default, fftblockpad_default,
     ...
     >        ilog2, u)
            if (timers_enabled) call timer_start(T_transpose)
            call transpose_x_yz(3, 2, x1, x2, dims)
            if (timers_enabled) call timer_stop(T_transpose)
            call cffts2(-1, dims(1,2), dims(2,2), dims(3,2), 
     >                  x2, x2, scratch,
     >                  fftblock_default, fftblockpad_default,
     ...
     >        ilog2, u)
            if (timers_enabled) call timer_start(T_transpose)
            call transpose_x_y(2, 1, x2, x1, dims,
     >                         T_transxyloc, T_transxyglo,T_transxyfin,
     >                         np1, commslice2)
            if (timers_enabled) call timer_stop(T_transpose)
            call cffts1(-1, dims(1,1), dims(2,1), dims(3,1), 
     ...
     >        T_synch, T_init, T_max
      ...
      do k = 1, d3
         do jj = 0, d2 - fftblock, fftblock
            ...
            call cfftz (is, logd1, d1, y, y(1,1,2), u,
     >                  fftblock_default, fftblockpad_default,
     >           transblock, transblockpad,
     >                  fftblock, fftblockpad)
     ...
     >        T_synch, T_init, T_max
      ...
      do k = 1, d3
        do ii = 0, d1 - fftblock, fftblock
           ...
           if (timers_enabled) call timer_start(T_fftlow)
           call cfftz (is, logd2, d2, y, y(1, 1, 2), u,
     >                 fftblock_default, fftblockpad_default,
     >          transblock, transblockpad,
     >                 fftblock, fftblockpad)
           if (timers_enabled) call timer_stop(T_fftlow)
     ...
     >        T_synch, T_init, T_max
      ...
      do j = 1, d2
        do ii = 0, d1 - fftblock, fftblock
           ...
           if (timers_enabled) call timer_start(T_fftlow)
           call cfftz (is, logd3, d3, y, y(1, 1, 2), u,
     >                 fftblock_default, fftblockpad_default,
     >          transblock, transblockpad,
     >                 fftblock, fftblockpad)
           if (timers_enabled) call timer_stop(T_fftlow)
...
c---------------------------------------------------------------------
      ...
      subroutine fft_init (n, u, seed, a, pi, alpha, ilog2)
...
c---------------------------------------------------------------------
      ...
      subroutine transpose_x_yz(l1, l2, xin, xout, dims)
...
c---------------------------------------------------------------------
      ...
      subroutine transpose_xy_z(l1, l2, xin, xout, np, dims,
     >                        T_transxzfin, np2, timers_enabled, 
     >                        T_transxzglo, commslice1,
     >                       transblockpad, transblock, T_transxzloc)
...
c      include 'global.h'
      ...
      integer T_transxzfin, T_transxzglo
      integer transblockpad, transblock, T_transxzloc 
      integer commslice1
      logical timers_enabled
      integer np, np2
      integer dims(3,3)
      ...
      call transpose2_local(dims(1,l1)*dims(2, l1),dims(3, l1),
     >                          xin, xout,
     >                     transblockpad, transblock, timers_enabled,
     >                     T_transxzloc)
      call transpose2_global(xout, xin, np,
     >                timers_enabled, commslice1, T_transxzglo)
      call transpose2_finish(dims(1,l1)*dims(2, l1),dims(3, l1),
     >                          xin, xout,
     >                              T_transxzfin, np2, timers_enabled)
...
c      include 'global.h'
      ...
      integer transblockpad, transblock
      integer T_transxzloc
      logical timers_enabled
...
c---------------------------------------------------------------------
      ...
      subroutine transpose2_global(xin, xout, np,
     >                timers_enabled, commslice1, T_transxzglo)
...
c      include 'global.h'
      ...
      integer transblock, transblockpad
      integer fftblock, fftblockpad
      integer commslice1, T_transxzglo, T_transxzfin
      integer np2, np1, np
      integer T_transxzloc
...
c---------------------------------------------------------------------
      ...
      subroutine transpose_x_y(l1, l2, xin, xout, dims,
     >                         T_transxyloc, T_transxyglo,T_transxyfin, 
     >                         np1, commslice2)
...
c      include 'global.h'
      ...
      integer dims(3,3)
      logical timers_enabled
      integer T_transxyloc, T_transxyglo, T_transxyfin
      integer np1
      integer commslice2
...
c---------------------------------------------------------------------
      ...
      subroutine checksum(i, u1, d1, d2, d3, sums,
     >                    timers_enabled, T_synch, 
     >                    xstart, ystart, zstart, me,
     >                    xend, yend, zend)
...
c---------------------------------------------------------------------
      ...
      subroutine verify (d1, d2, d3, nt, verified, class, sums, np, me)
